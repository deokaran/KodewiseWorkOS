...
export default async function TlKwTeamPage() {
  ...
  const filteredUsers = users.filter((u: any) => u.brand?.name === "Kodewise");
  const publishedProcesses = processes.filter((p: any) => p.versions.some((v: any) => v.isPublished));

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <h2 className="text-2xl font-bold tracking-tight text-gray-900 font-heading">Kodewise Team</h2>
        <p className="text-sm text-gray-500">
          Manage Kodewise developers, capability matrices, and assignments.
        </p>
      </div>
